...
Vector2 GameEntity::Pos(SPACE space) {
	...
	// The final position also depends on the parent's scale
...
void GameEntity::Parent(GameEntity* parent) {
	...
	// values, to keep the object looking rthe same after the removal of the parent
	if (parent == nullptr) {
		mPos = Pos(WORLD);
		mScale = Scale(WORLD);
		...
		// If the object already has a parent, remove the current parent to get it ready
		// to be the child of the new parent
		if (mParent != nullptr) {
			Parent(nullptr);
		}
		...
		// Setting the mew scale to be relative to the new parent
...
// float GameEntity::Rotation


void GameEntity::Scale(SPACE space) {
...
void GameEntity::Parent(GameEntity* parent) {
	if (parent == nullptr) {
		...
		mPos = RotationVector(Pos(WORLD) - parent->Pos(WORLD), parent->Rotation(WORLD));
		mPos.x /= parentScale.x;
		mPos.y /= parentScale.y;
	}
}
